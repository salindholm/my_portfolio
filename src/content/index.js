export default {
  nav: {
    logo: 'Sanne Lindholm',
    links: [
      { text: 'Stack', to: 'mystack' },
      { text: 'Work', to: 'mywork' },
      { text: 'Contact', to: 'mycontact' },
    ],
  },
  header: {
    img:
      'https://images.unsplash.com/photo-1504805572947-34fad45aed93?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1050&q=80',
    imgPlaceholder:
      'data:image/jpeg;base64,',
    text: ['Hi!', 'My name is Sanne.', "I'm a "],
    typical: [
      'Web Developer.',
      3000,
      'Front End Developer.',
      3000,
      'Back End Developer.',
      3000,
      'Full Stack Developer.',
      3000,
    ],
    btnText: 'Discover More',
  },
  stack: {
    title: 'Stack',
    tech: [
      {
        img: process.env.PUBLIC_URL + '/images/logo512.png',
        alt: 'react',
      },
      {
        img: process.env.PUBLIC_URL + '/images/ruby1.png',
        alt: 'rails',
      },
      {
        img: process.env.PUBLIC_URL + '/images/postgresql.png',
        alt: 'postgresql',
      },
      {
        img: process.env.PUBLIC_URL + '/images/semantic.png',
        alt: 'semantic',
      },
    ],
    desc: `Newly graduated from Craft Academy where I have been working in a fast-paced environment, learning how to work in real-life projects and situations. I got a solid knowledge of working with both front end and back end to build complete full stack applications, all based on the Agile methodology, Test-Driven Development process and pair programming.`,
  },
  contact: {
    title: 'My links!',
    desc: '',
    socials: [
      {
        alt: 'github',
        img: process.env.PUBLIC_URL + '/images/github.png',
        href: 'https://github.com/salindholm',
      },
      {
        alt: 'resume',
        img: process.env.PUBLIC_URL + '/images/resume.png',
        href: '../data/Resume.pdf',
      },
      {
        alt: 'linkedin',
        img: process.env.PUBLIC_URL + '/images/linkedin.png',
        href: 'https://www.linkedin.com/in/sanne-lindholm/',
      },
      {
        alt: 'email',
        img: process.env.PUBLIC_URL + '/images/email.png',
        href: 'mailto:sannelindholm@hotmail.com',
      },
    ],
    img: process.env.PUBLIC_URL + '/images/profile1.jpg',
    imgPlaceholder:
      'data:image/jpeg;base64,',
  },
  work: {
    title: 'Projects',
    img: process.env.PUBLIC_URL + '/images/profile.jpg',
    imgPlaceholder:
      'data:image/png;base64,',
    projectName: 'Sleeping',
    desc:
      'peaceful, and tranquil mean quiet and free from disturbance. calm is used when someone is not excited or upset even when there is cause for it. They stayed calm during the fire. peaceful is used when someone or something has reached a quiet state after some period of disturbance.',
  },
};
